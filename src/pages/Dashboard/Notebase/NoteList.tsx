 ...
 * @LastEditTime: 2025-01-21 09:47:26
...
// 智能体
const NoteList = ({ level, type }: NoteListProps) => {
  console.log("NoteList", level, type);
  ...
  const [noteResult, setNoteResult] = useState<NOTE.HttpPageResult>();
  const [currentNote, setCurrentNote] = useState<NOTE.NoteResponse>({});
  ...
  const handleDeleteRobot = async (robot: ROBOT.RobotResponse) => {
    ...
    if (response.data.code === 200) {
      ...
      const newData = [...noteResult?.data?.content as NOTE.NoteResponse[]];
  ...
  const handleListOnClick = (record: NOTE.NoteResponse, index: number) => {
    console.log("list on click", record, index);
    setCurrentNote(record);
  };

  const handleListOnEdit = (record: NOTE.NoteResponse, index: number) => {
  ...
  const handleListOnDelete = (record: NOTE.NoteResponse, index: number) => {
  ...
  return (
    <>
      ...
      <List
        dataSource={noteResult?.data.content}
        style={{ marginTop: 10 }}
        renderItem={(item, index) => (
          <List.Item
            key={item?.uid}
            style={
              currentNote?.uid === item.uid
                ? {
