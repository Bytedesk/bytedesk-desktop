 ...
 * @Date: 2024-07-11 12:29:20
 * @LastEditors: jackning 270580156@qq.com
 * @LastEditTime: 2024-12-23 17:14:45
...
const TabCaptcha = () => {
    ...
    useEffect(() => {
        //
        form.setFieldsValue({
            showCaptcha: agentInfo?.serviceSettings?.showCaptcha,
        });

    }, [agentInfo])

    const handleCaptchaSubmit = async (values: any) => {
        console.log('handleCaptchaSubmit:', values)
  
            const agentObject: AGENT.AgentRequest = {
                ...agentInfo,
                serviceSettings: {
                    ...agentInfo?.serviceSettings,
                    showCaptcha: values.showCaptcha,
        ...
        <div>
            ...
            >
                <ProFormSwitch
                    width={'md'}
                    name={'showCaptcha'}
                    label={intl.formatMessage({ id: 'pages.advanced.captcha', defaultMessage: 'Show Captcha' })}
                />
            </ProForm>
        </div>
    );
};
export default TabCaptcha;
