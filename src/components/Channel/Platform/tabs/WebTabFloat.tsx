 ...
 * @LastEditTime: 2024-12-20 17:42:16
...
import { getChatFloatJsUrl, getChatFloatUrl, getChatUrl } from "@/utils/configUtils";
...
const WebTabFloat = () => {
  ...
  //
  const codePreviewFloatUrl = useMemo(() => {
    let url = getChatFloatUrl() + "?org=" + currentOrg?.uid + "&t=" + THREAD_TYPE_AGENT + "&sid=" + agentInfo.uid;
  ...
  const openFloatUrl = () => {
    openUrl(codePreviewFloatUrl);
  };

  //
  const codeCopyFloatUrl = useMemo(() => {
    let chatUrl = getChatUrl() + "?org=" + currentOrg?.uid
    // 根据 showLang 和 lang 状态添加语言参数
    if (showLang) {
      chatUrl += "&lang=" + lang;
    }
    ...
    chatUrl += '&';
    //
    const scriptString = '<script src="' + getChatFloatJsUrl() + '"></script>\n'
    const configString = "<script>\n" +
      "window.ChatFloat({\n" +
      "chatUrl: '" + chatUrl + "',\n" +
      "});\n" +
      "</script>"
    return scriptString + configString;
  }, [currentRobot, agentInfo, showLang, lang, showTheme, theme]);

  const copyFloatUrl = () => {
    navigator.clipboard.writeText(codeCopyFloatUrl);
  ...
  return (
    <div>
      {/* <p>嵌入式客服代码</p> */}
      <TextArea style={{ width: 600 }} value={codeCopyFloatUrl} rows={7} />
      <br /><br />
      ...
      <br /><br />
      <Space>
        <Button key="openFloatUrl" type="primary" onClick={() => {
          openFloatUrl();
        }}>
          预览
        </Button>
        <Button key="copyFloatUrl" onClick={() => {
          copyFloatUrl();
        }}>
          复制
        </Button>
        <Button key="github" icon={<ArrowRightOutlined />} onClick={() => {
          openUrl('https://github.com/Bytedesk/bytedesk-web');
        }}>
          更多自定义及源代码，请参考：Github
        </Button>
      </Space>
    </div>
  );
}
export default WebTabFloat;
